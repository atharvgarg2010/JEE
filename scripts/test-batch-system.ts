import pg from "pg";
import { createStudent } from "@/lib/db/users";
import { assignTeacherToBatch, routeDoubtToTeacher } from "@/lib/db/batches";
...
async function runTests() {
  ...
  // 2. Fetch/choose a teacher to use for batch mapping (different from original author if possible)
  const teacherRes = await client.query(
    `SELECT id, username FROM users WHERE role = 'teacher' AND id != $1 LIMIT 1`,
    [testQuestion.teacher_id]
  );
  
  let mappedTeacherId: string;
  let mappedTeacherUsername: string;
